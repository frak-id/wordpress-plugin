...
class Frak_Webhook_Helper {
	...
	public static function send( $order_id, $status, $token ) {
		...
		try {
			...
			// Log webhook attempt.
			self::log_webhook_attempt( $order_id, $status, $http_code, $body, $execution_time, null );
			...
			$execution_time = round( ( microtime( true ) - $start_time ) * 1000, 2 );
			self::log_webhook_attempt( $order_id, $status, isset( $http_code ) ? $http_code : 0, isset( $body ) ? $body : '', $execution_time, $e->getMessage() );
